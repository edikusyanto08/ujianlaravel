    ...
    <div class="card-panel">
      <div class="row mar-bot">
        <div class="col s12 m12 l12">
          <h4 class="page-title">Tambah Mutasi Guru</h4>
        </div>
      </div>
      <div class="row mar-bot">
...
<div class="row">
  <div class="col s12 m12 l12 child-content">
    ...
    @endif
    <div class="card-panel">
      <form action="{{ route('mutasi_guru_store') }}" method="post" autocomplete="off">
        {{ csrf_field() }}
        <div class="row">
          <div class="input-field col s12 m12 l6">
            <select class="select2" name="guru_id">
              <option value selected disabled>Pilih Guru:</option>
              @foreach($guru as $value)
              <option value="{{ $value->id }}" @if(old('guru_id') == $value->id)selected @endif>{{ $value->nama }}</option>
              @endforeach
            </select>
          </div>
          <div class="input-field col s12 m12 l6">
